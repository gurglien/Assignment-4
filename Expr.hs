...
value (Num n) _ = n
value (Var v) dict 
	| isJust $ Dictionary.lookup v dict =  fromJust $ Dictionary.lookup v dict
	| otherwise = error "Non valid variable, please initiate first"
...
value (Div t u) dict 
	| value u dict /= 0 = value t dict `div` value u dict
	| otherwise = error "Can't divide by zero"
